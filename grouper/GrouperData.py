...
class GrouperDataset:
            ...
            for sample in samples:
                ...
                if variant_length:
                    ...
                    for _ in range(n_augmentations):
                        ...
                        toponym_id_sorted_in_source_ = toponym_id_sorted_in_source_
    ...
    def get_train_test_set(self, train_ratio=0.9, sample_ratio=1.0, random_seed=42):
        ...
        self.train_set = self.yield_samples(train_img_ids, sample_ratio=1.0, augmentation=True, variant_length=False)
        self.test_set = self.yield_samples(test_img_ids, sample_ratio=sample_ratio, augmentation=False, variant_length=False)
    ...
    def predict_plot(self,
        ...
        draw.line(list(zip(upper_half_x, upper_half_y)), fill="orange", width=5)
        draw.line(list(zip(lower_half_x, lower_half_y)), fill="orange", width=5)
        ...
        for i, pts in enumerate(neighbour_pts):
            ...
            draw.line(list(zip(upper_half_x, upper_half_y)), fill=color, width=5)
            draw.line(list(zip(lower_half_x, lower_half_y)), fill=color, width=5)
            draw.text((lower_half[0][0], lower_half[0][1]),
